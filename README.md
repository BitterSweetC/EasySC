...
- 画质可选 `2160p (4K)`、`1440p`、`1080p`、`不设上限（快速）`、`不设上限（高画质）`、`720p`
...
- `2160p / 1440p / 1080p / 720p`
  这些是“上限档位”。bridge 会优先选择“不超过该上限”的最高可用档位。
- 如果站点没有你选中的精确分辨率，例如只有 `1080p` 没有 `1440p`，则会自动退到更低但最接近的可用档位，通常就是 `1080p`。
- 如果上限以下一个候选都没有，bridge 才可能退到更高但最接近的可用档位。
- `不设上限（快速）`
  不限制分辨率上限，但更偏向优先选“能尽快直投/直转码”的单链接兼容流。启动通常更快，也更不容易先在本地完整下载分离流，但最终清晰度不一定最接近浏览器里实际播放的最高画质。
- `不设上限（高画质）`
  不限制分辨率上限，并且在 Bilibili、腾讯视频这类常见 DASH 页面上，会更偏向优先解析分离音视频流。这样更接近网页里的高画质结果，但更容易先在本地下载再合流或转码，启动时间通常会更长。

### 画质与投屏模式的关系

- “画质”决定的是：优先取哪一档源、是否限制分辨率上限，以及“不设上限”时更偏快还是更偏画质。
- “投屏模式”决定的是：bridge 对拿到的源做多少本地处理。
- `原样优先`
  更偏向尽快开始播放。即使如此，遇到 Bilibili 这类分离 DASH 音视频流页面时，仍然可能需要先在本地下载音视频再合并成兼容 MP4。
- `画质优先`
  会优先准备更兼容、通常也更清晰的本地版本。对远端 `HLS(.m3u8)` 或不兼容编码，这通常意味着本地转码。
- 即使是 `画质优先`，bridge 也不会一律完整重编码。对于已经是 `H.264 + AAC` 的兼容流，会先尝试快速无重编码合流；只有不兼容时才进入完整本地优化。
...
- 从源码运行 bridge 时，本地下载、合流和转码缓存默认放在：
  `cache\compat_media\remote_mux`、`cache\compat_media\remote_transcode`、`cache\compat_media\live_transcode`
- 对 Bilibili 分离流，常见文件形态包括：
  最终的 `*.mp4`，以及中间下载的 `*.video.m4s`、`*.audio.m4s`
- 相同源再次投屏时，如果对应缓存文件仍存在且非空，bridge 会优先复用旧缓存，不会每次都重新下载。
- 当前打包版 `ScreenCastingBridge.exe` 是 PyInstaller onefile 形式。按当前实现，bridge 相关缓存路径不一定在 `exe` 同级目录，可能出现在运行时临时解包目录中，因此不建议把这些缓存当作长期保存位置。

### Bilibili 使用注意事项

- Bilibili 很多清晰度档位本身就是分离的 DASH 音视频流，不是单个可直接投到电视的 MP4。
- 这类页面在 `不设上限（高画质） + 画质优先` 下，更容易出现“正在下载视频，准备高画质可兼容版本...”这样的提示，这是预期行为，不是异常。
- 如果你更在意启动速度，优先尝试 `1080p + 原样优先`，或者 `不设上限（快速） + 原样优先`。
- 如果你更在意尽量接近网页里的清晰度，优先尝试 `2160p / 1440p / 1080p + 画质优先`，或者 `不设上限（高画质） + 画质优先`。
